=== saitama ===
Tags: two-columns, right-sidebar, responsive, custom-colors, custom-menu, theme-options

Requires at least: 4.0
Tested up to: 6.3.1
Stable tag: 1.2.5
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html


== Description ==
Saitama is a simple WordPress theme bulit by Bootstrap. It supports tablets and smartphones in the responsive Web design. By theme customize, setting slider you can easily.


== Features ==
Bootstrap based,Supports customizer, Responsive Layout, Page Navigation, Breadcrumb Navigation, and more!


=== Widgets Areas ===
The Theme has 7 widget areas.
You can use these area to customize the content of your website.


== License ==
Unless otherwise specified, all the theme files, scripts and images are licensed under GNU General Public License version 2, see file license.txt.
The exceptions to this license are as follows:
The script html5.js and css3-mediaqueries.js are licensed under MIT
Header Images are photo by theme author and are released under GPL license.

== Third Party Resources ==

html5shiv.js
License: MIT/GPL2 License
Source: https://github.com/aFarkas/html5shiv

respond.js
License: MIT/GPLv2 License
Source: https://github.com/scottjehl/Respond

Animate.css
License: MIT License
Source: http://daneden.me/animate

Font Awesome
License: SIL Open Font License, MIT License
Source: http://fontawesome.io/

Simple line icons
License: MIT License
Source: https://github.com/thesabbir/simple-line-icons

hover.css
License: MIT License
https://github.com/IanLunn/Hover

== Author ==
The theme built by Yuusuke Yoshida at Communitycom,Inc.
You can contact me at inquiry form.
http://www.communitycom.jp/mail/


== Changelog ==
v1.2.2
* CSS adjustment

v1.2.1
* Bootstrap update to v3.4.1

v1.2.0
* Test Up to 5.1.0

v1.1.1
* PHP tag bug fix

v1.0.9
* <html>tag bug fix
  Add css for print-style

v1.0.8
* Change Footer Credits

v1.0.7
* Frontpage Image Style changed.

v1.0.6
* Tag Cloud Widgets bug fix
  Mobile Menu bug fix

v1.0.5
* Toparea Style bug fix

v1.0.4
* Delete Header Widget
* Add Default Widget Style

v1.0.3
* Change Top H2 Style

v1.0.2
* Edit link bug fix

v1.0.1
* Change Default Thumbnail Image

v1.0.0
* Initial release
